...
export default function ForecastCard({ forecasts, unit, className = "" }: ForecastCardProps) {
  ...
  return (
    <Card className={`p-6 sm:p-8 card-hover gradient-bg border-0 shadow-2xl w-full max-w-4xl bg-gradient-to-br from-white/10 via-white/5 to-transparent backdrop-blur-md ${className}`} data-testid="card-forecast">
      <div className="space-y-4 sm:space-y-5">
        <h3 className="text-xl sm:text-2xl font-semibold text-foreground mb-6 sm:mb-8">7-Day Forecast</h3>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-3 sm:gap-4 md:gap-5">
          {forecasts.map((forecast, index) => {
            ...
            return (
              <div
                key={forecast.date}
                className={`relative p-4 sm:p-3 md:p-4 rounded-xl transition-all duration-700 ease-out hover:shadow-2xl hover:scale-[1.03] backdrop-blur-md overflow-hidden group ${
                  isToday
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-center justify-between mb-3 sm:mb-4">
                    <div className="flex items-center space-x-3 sm:space-x-4">
                      ...
                      <WeatherIcon className={`w-5 h-5 ${getWeatherColor(forecast.condition)}`} />
                    </div>
                  ...
                  <div className="flex items-center justify-between mb-3 sm:mb-4">
                    ...
                    <div className="flex items-center space-x-3 sm:space-x-4">
                      <div className="relative w-16 sm:w-14 h-2.5 sm:h-2 bg-gradient-to-r from-white/20 to-white/10 rounded-full overflow-hidden backdrop-blur-sm shadow-inner">
                        <div
